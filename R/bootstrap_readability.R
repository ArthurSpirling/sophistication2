...
bootstrap_readability <- function(x, measure = "Flesch", n = 100, ..., verbose = TRUE) {
    ...
    for (i in 1:n) {
        # Resample sentences with replacement (preserving duplicates)
        sent_sample <- sample(seq_len(n_sent), size = n_sent, replace = TRUE)
        boot_texts <- sent_texts[sent_sample]

        # Combine sampled sentences into a single bootstrap document
        corp_boot_doc <- quanteda::corpus(paste(boot_texts, collapse = " "))
